    ...
    foreach ($sort as $s) {
        ...
        if (isset($_GET[$s])) { // if the field is set
            if ($_GET[$s] == 'all') // if "all", use wildcard
            {
                ...
                $sortarray[$s] = RobotessNet\StringUtils::instance()->clean($_GET[$s]);
            }
            $sortselectednum++;
        } else // use wildcard
        {
...
if (isset($hide_members_until_final) && $hide_members_until_final && count($sortarray) > $sortselectednum) {
    return;
}
...
if (isset($no_sort) && $no_sort) {
    ...
    foreach ($sort as $s) {
        ...
        $showstring .= ': ';
        $showstring .= StringUtils::instance()->clean($_GET[$s]);
        $showstring .= ', ';
    }
...
foreach ($_GET as $key => $value) {
    if ($key != 'start' && $key != 'PHPSESSID') {
        $url .= $connector . StringUtils::instance()->clean($key) . '=' . StringUtils::instance()->clean($value);
        $connector = '&amp;';
    }
}
